...
生成脚本前，Worker 使用测试用例名称、前置条件和步骤定位当前分支源码。没有项目仓库、对应 PAT、可读相关源码时，任务直接失败，不启动认证环境、页面探测或脚本生成。代码预检只证明相关实现可能存在，不证明行为正确；通过预检后，有限代码上下文会辅助模型理解入口、路由、角色和状态，真实页面探测和最终测试执行仍负责验证实际行为。代码上下文不会保存到 TestRun，也不会把完整源码写入执行日志。
