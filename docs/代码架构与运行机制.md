...
| `Project`                  | 名称、描述、Base URL、自动化约束、两类 PAT 密文和账号            |
...
| `TestCase`                 | TC 编号、唯一可选 US、分组、优先级、自然语言内容、脚本及脚本来源 |
...
- 删除项目：当前保护条件是项目中不存在有效 FE、US 或测试用例；PAT 同时清空。
...
不包含变量值。这样更换密码不会让脚本无意义过期，但变量名或用途变化会触发更新。
...
- 需要生成脚本的任务会使用 Worker 启动时的用例内容、变量和自动化约束。
- 变量值不会被长期复制到 TestRun。
...
4. [`src/automation/playwright-cli-session.ts`](../src/automation/playwright-cli-session.ts)
5. [`src/automation/script-validator.ts`](../src/automation/script-validator.ts)
6. [`src/runner/worker.ts`](../src/runner/worker.ts)
7. [`src/runner/generate-script.ts`](../src/runner/generate-script.ts)
8. [`src/runner/run-execution.ts`](../src/runner/run-execution.ts)
