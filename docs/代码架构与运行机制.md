...
- 自动化脚本生成前会读取项目配置分支的最新代码；代码预检失败时不会启动页面探测。
...
| 密码     | Argon2id                                    | 密码哈希和验证                               |
| 密文     | Node.js AES-256-GCM                         | 开启加密的项目变量、PAT、模型 API Key        |
...
src/ai/
  ...
  repository-access.ts       项目仓库与 PAT 解析
...
src/automation/
  ...
  authentication.ts          登录方法校验、账号对象解析和临时认证状态
  code-readiness.ts          自动化脚本生成前的当前代码预检和上下文限制
...
- `APP_ENCRYPTION_KEY` 解密已加密的项目变量、GitHub/Gitee PAT 和模型 API Key。
...
| `/project-settings`               | 项目基本信息                         |
| `/project-settings/repositories`  | PAT 和代码仓库                       |
| `/project-settings/testing`       | Base URL、变量、登录复用和自动化约束 |
| `/projects`                       | 项目管理                             |
| `/ai-settings`                    | 管理员模型配置                       |
| `/users`                          | 管理员用户管理                       |
...
erDiagram
  ...
  ProjectVariable ||--o{ ProjectVariableField : 包含
...
- 测试用例必须属于一个有效分组。
- 项目变量可以是字符串、数字或一层对象；标量和对象字段通过独立开关决定是否加密。
- 用例通过 `${NAME}` 或 `${NAME.field}` 引用变量；完整账号对象引用会触发任务内预登录。
- 测试用例可以有多次 `TestRun`。
...
| `ProjectVariable`          | 名称、字符串/数字值或密文、描述、类型、加密开关和顺序               |
| `ProjectVariableField`     | 对象变量的一层字符串/数字字段、值或密文、描述、加密开关和顺序       |
...
| `TestCase`                 | TC 编号、唯一可选 US、分组、自然语言内容、脚本及来源                |
...
| `TestCaseDraft`            | 可独立评审并选择分组的单条测试用例草稿                              |
...
| 脚本来源     | `MANUAL`、`AI`                                                  |
| 项目变量类型 | `STRING`、`NUMBER`、`OBJECT`                                    |
| 对象字段类型 | `STRING`、`NUMBER`                                              |
...
- 项目变量支持在测试设置中新增和编辑；删除或修改变量结构时若有效正式用例、待评审用例或脚本存在无效引用，则拒绝并指出引用位置。
- 修改变量类型或对象字段：会在事务提交前重新校验全部引用；不自动改写业务文本。
- 删除项目：当前保护条件是项目中不存在有效 FE、US 或测试用例；PAT 与登录方法同时清空。
...
- 可选 Base URL。
- 仅包含当前项目差异的自动化约束；通用安全与脚本规则由内置提示词维护。
- 字符串、数字或一层对象项目变量，以及各值的加密开关。
- 可选公共登录方法。

任务运行时为每个变量路径生成内部环境键，脚本只能通过只读的 `getVariable()` / `getCredentials()` 助手读取；写操作使用同一变量助手提供的 `getUniqueValue()` 生成本次运行唯一数据；另有固定的 `BASE_URL`。

变量规则：

- 字符串和数字标量使用 `ProjectVariable.value`；对象变量的顶层值始终为空。
- `encrypted` 只决定对应值是否以 AES-256-GCM 密文存储，不改变变量的数据类型。
- 对象只支持一层字符串或数字字段，不支持数组、嵌套对象或任意 JSON。
- 标量和对象字段各自通过 `encrypted` 开关决定是否保存 AES-256-GCM 密文；加密值永不回显。
- 变量创建后不可修改名称、类型、值、加密状态、描述或对象字段，只能删除后重新创建。
- 保存事务仅接受保留的已有变量 ID和完整的新变量定义，服务端不会信任客户端提交的已有变量内容。
- 删除前校验正式用例、待评审用例和脚本中的变量引用，避免产生悬空引用。

### 13.2 登录复用

登录复用与 Base URL、项目变量和自动化约束统一在“测试设置”页面维护。每个项目可以配置一份公共 Playwright TypeScript 登录方法；账号数据由对象变量提供，不建立专用身份表。
...
- `${LOCALE}`：引用字符串或数字标量。
- `${ORDER.id}`：引用对象字段。
- `${ADMIN}`：把完整账号对象交给公共登录方法。该对象必须包含字符串字段 `username`、`password`，且 `password` 开启加密。

完整对象引用只用于成功登录，一条用例最多使用一个账号对象。登录失败、退出等认证功能用例应使用 `${ADMIN.username}` 等字段引用或明确测试数据，由用例自身操作认证页面，不触发预登录。模型只获得变量结构和描述，不获得任何值。
...
自动化脚本提示词目录中的 `check-code.md` 负责按路径筛选候选源码，`verify-code.md` 负责读取源码后确认测试用例要求的具体行为是否确实实现；两步都通过后才进入页面探测。
...
| 主动生成自动化脚本 | 等待执行 → 检查代码实现 → 探测页面 → 生成脚本 → 校验脚本 → 已完成          |
| 测试用例执行       | 等待执行 → 可选检查代码实现/生成脚本/探测页面/校验脚本 → 执行测试 → 已完成 |
...
运行没有脚本的测试用例时，会同时创建一条 TestRun 和一条 `GENERATE_AUTOMATION_SCRIPT` AI 任务。调度器先领取 AI 任务，成功后把脚本快照交给 TestRun，再领取测试执行；两者按同一个浏览器资源池调度，不会重复生成脚本。
...
- 点击“运行”创建一个 `TestRun`。
- 如果没有可用脚本，同一事务额外创建一条 `GENERATE_AUTOMATION_SCRIPT` AI 任务。
- 调度器在 AI 任务成功后把脚本写入待运行 TestRun；AI 任务失败时，TestRun 同步失败，不启动页面探测或测试执行。
...
- 一条正式自然语言测试用例。
- 项目 Base URL。
- 项目变量及对象字段的名称、类型、加密状态和描述。
- 项目公共登录方法的固定契约。
- 当前项目特有的自动化约束。
- 项目配置分支最新提交中与测试用例相关的有限源码。
- 自动化 Skill 和提示词。
- Playwright CLI 对真实页面的探测结果。

生成脚本前，Worker 使用测试用例名称、前置条件和步骤定位当前分支源码，并再次检查已读取源码是否直接实现用例要求的具体行为。仅找到通用页面、路由壳或列表容器不算实现；例如用例要求显示“创建人”而源码没有对应字段时，任务会在页面探测前失败。没有项目仓库、对应 PAT、可读相关源码或直接实现证据时，任务直接失败，不启动认证环境、页面探测或脚本生成。通过预检后，有限代码上下文只辅助模型理解入口、路由、角色和状态，真实页面探测和最终测试执行仍负责验证实际行为。代码上下文不会保存到 TestRun，也不会把完整源码写入执行日志。

### 20.3 认证准备与登录复用

用例包含完整账号对象引用（例如 `${ADMIN}`）时，在调用模型前执行：
...
sequenceDiagram
  ...
  W->>D: 读取登录方法、变量结构和引用值
  W->>W: 校验账号对象、解密字段并登记脱敏值
  W->>S: 写入内部变量助手、login.ts 和认证 Setup
  S->>S: 调用 login(page, credentials)
  S-->>W: 生成任务内 storageState
  W->>C: 使用 storageState 创建隔离探测会话
  C-->>M: 返回已登录页面的安全探测结果
  M-->>W: 提交只包含业务步骤的脚本
  W->>W: 校验脚本引用公共 login 方法和账号对象
```

关键约束：

- 登录方法缺失、账号对象字段失效、密码未开启加密或密文无法解密时，任务在模型调用前失败。
- 认证 Setup、`storageState`、CLI 会话和生成校验各自位于当前任务目录。
- `storageState` 与变量助手只服务当前任务，不入库、不写日志、不跨任务缓存。
- 模型只知道变量名称、字段结构、描述和公共方法契约，不接收登录源码、用户名、密码或 `storageState`。
- 最终脚本通过 `getCredentials("ADMIN")` 读取账号并调用 `./specchain/login`；Runner 执行时写入项目当前版本的公共登录方法。
- 登录、退出、错误密码等认证功能用例只引用对象字段，不引用完整账号对象，由用例本身正常探测和验证认证流程。

没有完整账号对象引用的用例跳过认证 Setup，生成脚本不得导入或调用公共登录方法。若普通业务用例被登录页阻挡，任务会提示在用例中使用已配置的账号对象。
...
- 变量名。
- 字符串/数字/对象类型、加密状态及对象字段结构。
- 描述。

真实变量值存放在服务端 AI SDK `toolsContext` 中：

- 模型调用 `fillField` 或 `selectOption` 时只指定变量路径。
- 工具执行器在服务端解析真实值。
- CLI 输出在返回模型和写日志前再次脱敏。
- 最终脚本只能使用字面量路径调用 `getVariable()` / `getCredentials()`，写操作只能使用 `getUniqueValue()` 生成运行时数据，不能直接读取项目变量环境键或写入探测值。
...
- 探测会话通过浏览器 `initScript` 阻断非 GET/HEAD/OPTIONS 请求；探测可以读取和完成只读交互，但不会把提交、编辑、删除等业务写请求发送到服务端。
- 页面探测命令单次超时 60 秒。
...
- 只操作本次运行创建的唯一临时数据。
- 使用变量助手的 `getUniqueValue("稳定前缀")` 生成运行时唯一标识；Runner 为每个 TestRun 注入独立标识。
- 不修改或删除已有业务数据。
- 使用 `try/finally` 清理本次创建的数据。

代码还使用中文写操作关键词做保守检测。若用例看起来包含新增、编辑、删除、状态变化等操作，而生成脚本没有 `try/finally` 或 `getUniqueValue`，静态校验直接失败。
...
specchain/variables.ts
specchain/login.ts     # 仅完整账号对象用例
```
...
- Base URL。
- 项目自动化约束。
- 用例实际引用变量的名称、类型、描述和对象字段结构。

不包含变量值，也不包含公共登录方法源码。这样更换账号密码或优化公共登录实现不会让所有业务脚本无意义过期；修改被引用变量的名称、类型、描述或对象字段结构会触发更新。
...
运行时自动补齐脚本的 AI 任务来源固定为 `TEST_RUN`，只有该来源可以同步当前 TestRun 的阶段日志和脚本快照；用户主动生成脚本的 `USER` 任务保持独立，不会串入待运行 TestRun。
...
sequenceDiagram
  ...
  API->>API: 计算当前脚本状态
  alt 脚本快照为空
    API->>D: 同事务创建 QUEUED TestRun 和 AI 脚本任务
  else 已有可用脚本
  ...
  alt 先领取 AI 脚本任务
    S->>A: 启动独立 AI Worker
    A->>D: 代码预检、页面探测、生成并保存脚本
    A->>D: 更新待运行 TestRun 的脚本快照
    S->>R: 启动独立 Runner
  else 直接领取已有脚本的 TestRun
    S->>R: 启动独立 Runner
  end
  R->>D: 读取运行记录、当前变量和变量引用
  opt 脚本调用公共登录方法
    R->>L: 写入当前登录方法和变量助手
  end
...
| 项目变量结构和值   | Runner 真正开始执行时读取                |
| 用例中的变量引用   | Runner 真正开始执行时校验                |
...
- 需要生成脚本的任务会使用 Worker 启动时的用例内容、变量结构和自动化约束。
- 公共登录方法不进入脚本快照；完整账号对象用例执行时始终加载项目当前实现。
- 变量值不会被长期复制到 TestRun。
...
- 只读 `specchain/variables.ts`，以及账号用例所需的 `specchain/login.ts`。

项目变量值只存在于 Worker 内存和 Playwright 子进程环境中，并全部注册到日志脱敏器；开启加密的值在使用前才解密。
...
- 同源页面限制。
- 项目变量值通过服务端工具上下文使用。
- 登录账号只引用 password 已开启加密的对象变量，不重复保存凭据。
...
- 生成 US、生成测试用例工作流。
- 提示词模板和安全边界。
- 登录方法契约、对象变量解析和 Playwright 编译检查。
- 自动化指纹、变量引用、同源限制和脚本静态校验。
- 调度池容量和 FIFO 策略。
- 运行日志脱敏和终态映射。
...
1. 测试用例是否使用了正确的完整账号对象引用，例如 `${ADMIN}`。
2. 项目是否保存了可通过编译检查的公共登录方法。
3. 账号对象是否仍包含 `username` 和敏感的 `password` 字段。
4. 两个字段是否有值，敏感值能否用当前 `APP_ENCRYPTION_KEY` 解密。
