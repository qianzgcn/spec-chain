...
- 每个项目可维护一份公共登录方法和多个账号身份，自动化脚本无需重复生成登录流程。
...
| 生成自动化脚本 | 正式用例、Base URL、变量元数据、登录身份、自动化约束 | 模型工具调用、Playwright CLI | 校验后保存的 Playwright 脚本 |
...
- 一份固定接口的 Playwright TypeScript `login(page, credentials)` 方法。
- 多个登录身份，每个身份只映射现有的用户名变量和敏感密码变量。
- 每条正式用例和待评审用例最多选择一个身份；认证流程用例选择“不预登录”。

账号用例生成脚本前，Worker 会在当前任务的独立目录中执行公共登录方法，生成临时 `storageState`，再让 Playwright CLI 探测已登录页面。模型只获得公共方法契约和变量名，不获得登录源码或变量值；最终脚本统一引用 `./specchain/login`。正式执行时 Runner 写入项目当前登录方法，因此登录实现更新后无需重新生成所有业务脚本。

`storageState` 不入库、不写日志、不跨任务缓存，任务结束即随临时目录删除。账号或变量映射改变会使 AI 脚本进入“需更新”；公共登录方法源码改变不会让业务脚本过期。
...
2. Runner 读取当前登录身份及其变量；配置失效时在模型调用前失败。
