...
- 使用 `process.env.BASE_URL` 导航；标量和对象字段分别通过 `getVariable("NAME")`、`getVariable("NAME.field")` 读取；需要创建临时数据时使用 `getUniqueValue("稳定前缀")` 生成每次运行唯一的名称或编号，不得把探测时使用的真实值写入脚本。
- 只要使用 `getVariable`、`getCredentials` 或 `getUniqueValue`，就必须使用提示词提供的完整变量助手导入行，不得改写导入名称或路径。
